        ...
        <div class="flex justify-between flex-col md:flex-row gap-4">
            <div class="flex flex-col gap-4 flex-1">
                <label class="input w-full" for="nickname">
                    ...
                    <input type="text" name='nickname' value="{{ $sectionData['user']->nickname }}" required
                        placeholder="Nickname" />
                </label>
                <label class="input w-full" for="real_name">
                    <span class="label">Real Name</span>
                    <input type="text" name='real_name' value="{{ $sectionData['user']->real_name }}" required
                        placeholder="Real Name" />
                </label>
                <label class="textarea w-full" for="bio">
                    <span class="label">Bio</span>
                    <textarea name="bio" placeholder="Bio" class="textarea w-full">{{ $sectionData['user']->bio }}</textarea>
                </label>
                    ...
                    <select name="country_id" required>
                        @foreach ($countries as $country)
                            <option value="{{ $country->id }}"
                                {{ $sectionData['user']->country_id == $country->id ? 'selected' : '' }}>
                                {{ $country->name }}
                            </option>
                        @endforeach
                    </select>
